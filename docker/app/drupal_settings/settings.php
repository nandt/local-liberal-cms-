...
if (strcasecmp(getenv("S3_USE_CNAME"), "true") == 0) {
    $config['s3fs.settings']['use_cname'] = TRUE;
    $config['s3fs.settings']['domain'] = getenv("S3_CNAME");
    $config['s3fs.settings']['domain_root'] = "none";
}
if (strcasecmp(getenv("S3_CUSTOM_ASSET_HOST"), "true") == 0) {
  $config['s3fs.settings']['use_cssjs_host'] = TRUE;
  $config['s3fs.settings']['cssjs_host'] = getenv("BUCKET_NAME");
}
if (strcasecmp(getenv("S3_USE_FOR_PUBLIC_FOLDER"), "true") == 0) {
    $settings['s3fs.use_s3_for_public'] = TRUE;
    $config['s3fs.settings']['public_folder'] = "public";
}
if (strcasecmp(getenv("S3_USE_FOR_PRIVATE_FOLDER"), "true") == 0) {
    $settings['s3fs.use_s3_for_private'] = TRUE;
    $config['s3fs.settings']['private_folder'] = "private";
}
if (strcasecmp(getenv("S3_USE_HTTPS"), "true") == 0) {
...
# Inbroken
$settings['inbroker.postUrl'] = getenv("INBROKER_POST_URL");
$settings['inbroker.userName'] = getenv("INBROKER_USERNAME");
$settings['inbroker.sessionID'] = getenv("INBROKER_SESSIONID");
$settings['inbroker.company'] = getenv("INBROKER_COMPANY");
$settings['inbroker.lang'] = getenv("INBROKER_LANG");
$settings['inbroker.market'] = getenv("INBROKER_MARKET");
$settings['inbroker.type'] = getenv("INBROKER_TYPE");

# Change location of twig cache. Recomended when using s3fs for public folder
# https://git.drupalcode.org/project/s3fs
$settings['php_storage']['twig']['directory'] = '/opt/drupal/twig_cache';
...
if (getenv('DISABLE_JSONAPI_NORMALIZATION_CACHE') === 'TRUE') {
