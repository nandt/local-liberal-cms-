...
namespace Drupal\unicorn_sources\Hook;
...
final readonly class SourcesHooks {

  public function __construct(
    private SourceUsageChecker $sourceUsageChecker,
  ) {}
  ...
  public function entityBaseFieldInfoAlter(array &$fields, EntityTypeInterface $entityType): void {
    ...
    if (isset($fields['name'])) {
      $fields['name']->addConstraint('UniqueSourceName');
    }
  }
  ...
  public function taxonomyTermAccess(TermInterface $term, string $operation, AccountInterface $account): AccessResultInterface {
    if ($operation !== 'delete' || $term->bundle() !== 'piges_arthron') {
      return AccessResult::neutral();
    }

    return AccessResult::forbiddenIf($this->sourceUsageChecker->isUsedByPublishedArticle($term))
